...
void user::c_data::log(std::string txt, bool error)
{
	...
	date = std::string(std::to_string(ltm->tm_mday)).append("_").append(std::to_string(ltm->tm_mon + 1)).append(stra("_launcher_r0.txt"));
...
{
	auto ky = this->openkey(HKEY_LOCAL_MACHINE, stra("SOFTWARE\\somethingE"));
